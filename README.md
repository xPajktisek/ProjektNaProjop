# 🎮 **Retro Arcade** 🕹️

Witaj w **Retro Arcade** – neonowym portalu do złotej ery gier wideo! ✨
To tu kultowe klasyki z lat 70., 80. i początku 90. powracają w odświeżonej, pikselowej odsłonie.
Całość obsługujesz **prostymi klawiszami**, a klimat retro wylewa się z każdego kąta ekranu.

---

## ⭐ **Co znajdziesz w Retro Arcade?**

Zestaw legendarnych minigier – każda inspirowana oryginałem, lecz przygotowana tak, by grało się przyjemnie i współcześnie.

### 🐍 **Snake** *(1976)*

Kultowy wąż powraca! Zbieraj jedzenie, rośnij i unikaj uderzenia w ogon.
**Status:** ✅ Dostępna

### 🧩 **Tetris** *(1984)*

Układaj spadające klocki, czyść linie i zdobywaj punkty.
**Status:** ✅ Dostępna

### 🏓 **Pong** *(1972)*

Pierwsza gra wideo w historii! Odbijaj piłeczkę i pokonaj przeciwnika.
**Status:** ✅ Dostępna

### 👾 **Space Invaders** *(1978)*

Zestrzel nadciągające hordy kosmitów i ocal Ziemię. Uważaj – kosmici strzelają!
**Status:** ✅ Dostępna (3 życia, strzały wrogów)

### 👻 **Pac-Man** *(1980)*

Unikaj duchów, zbieraj kulki i biegnij przez labirynt w jednym z największych hitów wszech czasów.
Zjadaj power pellety, by duchy zmieniły się w ofiary!
**Status:** ✅ Dostępna (labirynt, power pellety, duchy frightened, 3 życia)

---

## 🎨 **Sterowanie**

| Akcja             | Klawisz           |
| ----------------- | ----------------- |
| Nawigacja po menu | ⬆️⬇️              |
| Wybór gry         | **ENTER**         |
| Wyjście           | **ESC**           |
| Ruch (w grze)     | **Strzałki**      |
| Strzelanie (SI)   | **Spacja**        |
| Obrót (Tetris)    | **Spacja**        |

Prosto, szybko, klasycznie – tak jak w starych automatach arcade. 🕹️


| Kontekst | Klawisz | Akcja |
|----------|---------|-------|
| Menu | ↑ / ↓ | Nawigacja po liście gier |
| Menu | Enter | Uruchomienie wybranej gry |
| Snake | ← → ↑ ↓ | Zmiana kierunku węża |
| Tetris | ← → | Przesuw klocka | 
| Tetris | ↓ | Przyspieszenie spadania |
| Tetris | Spacja | Obrót klocka |
| Pong | ↑ / ↓ | Ruch paletką |
| Space Invaders | ← → | Ruch statkiem |
| Space Invaders | Spacja / ↑ | Strzał laserem |
| Pac-Man | ← → ↑ ↓ | Kierunek ruchu |
| Wszystkie | Escape | Powrót do menu |

---

## 🌐 **Technologie**

* HTML5 + Canvas 2D
* JavaScript (Vanilla)
* CSS3 z animacjami i efektem CRT
* Google Fonts: *Press Start 2P*
* LocalStorage – zapis wyników, monet, motywów

---

## 💰 **System monetyzacji**

* **Daily Bonus** – 5000 monet codziennie na start
* **Monety za granie** – zdobywaj monety w każdej grze
* **Sklep z motywami** – 6 kolorystycznych motywów: Cyber, Classic, Sunset, Matrix, Vaporwave, Arctic

---

## ⚡ **Planowane aktualizacje**

* Efekty dźwiękowe i muzyka w klimacie retro
* Animacje i efekty wizualne w stylu neon-pixel
* Tablice rekordów online 🏆
* Możliwe rozszerzenie o kolejne klasyki (Breakout, Frogger)

---

## 📋 **Historia aktualizacji**

| Wersja | Data | Opis |
|--------|------|------|
| 1.0.0 | 2026-03-18 | Wszystkie 5 gier w pełni grywalne. Pac-Man z labiryntem, power pelletami i systemem żyć. Space Invaders z ogniem wrogów. |
| 0.7.0 | 2026-02-18 | Debugowanie Etap 5 – naprawa krytycznego błędu TypeError w `startGame()` |
